...
const refreshTiktokRatesForAllAccounts = (req, res) => {
      cors(req, res, async () => {
        ...
        usersRef.orderByChild('creator_socials/tiktok/performance/suggestedRate').startAt(null).once('value')
        .then(snapshot => {
          snapshot.forEach(userSnapshot => {
            const userId = userSnapshot.key;
            //console.log(userId);
            const tikTokInfo = userSnapshot.child('creator_socials/tiktok/performance/suggestedRate').val();
        ...
        res.status(200).json({ message: 'Suggested rates updated succesfully' });
        })
        .catch(error => {
    ...
    const getSuggestedRateTikTok = async (userId) => {
      try {
        ...
        const snapshot = await firebase.database().ref(`users/${creator_id}/creator_socials/tiktok/access_token`).once('value');
        ...
        const userInfoResponse = await fetch('https://open.tiktokapis.com/v2/user/info/?fields=follower_count', { method: 'GET', headers: userInfoHeaders });
        ...
        const videoListResponse = await fetch('https://open.tiktokapis.com/v2/video/list/?fields=like_count,share_count,view_count,comment_count,share_url', videoListOptions);
        ...
        await firebase.database().ref(`users/${creator_id}/creator_socials/tiktok/handle`).set(handle);
      }
      ...
      const median_comments = median(commentArray);
        ...
        */
        ...
        await firebase.database().ref(`users/${creator_id}/creator_socials/tiktok/performance`).set({
          median_likes,
          median_shares,
          followerCount,
          median_views,
