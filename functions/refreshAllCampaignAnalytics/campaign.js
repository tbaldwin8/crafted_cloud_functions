...
const refreshAllCampaignAnalytics = async (req, res) => {
  ...
  try {
    ...
    const snapshot = await firebase.database().ref('influencer_campaigns').once('value');
    ...
    for (const [campaign_id, campaign] of Object.entries(campaigns)) {
      ...
      for (const [task_id, task] of Object.entries(tasks)) {
       ...
       if (task_posts && Object.keys(task_posts).length > 0 && campaign.status != 'completed')  {    
        ...
        hasPosts = true;
          for (const [post_id, task_post] of Object.entries(task_posts)) {
            if (task_post.platform === "tiktok") {
              const video_id = task_post.link && task_post.link.split("/video/")[1]?.split("?")[0];
              const userSnapshotPromise = firebase.database().ref(`users/${task_post.creator_id}/creator_socials/tiktok/access_token`).once('value');
          ...
          if (task_post.platform === "instagram") {
            const userSnapshotPromise = firebase.database().ref(`users/${task_post.creator_id}/creator_socials/instagram/access_token`).once('value');
            const snapshot = await userSnapshotPromise;
            const accessToken = snapshot.val();
            if (accessToken) {
              const new_entry = {
              ...
              .database()
              .ref(`influencer_campaigns/${campaign_id}/tasks/${task_id}/posts/${post_id}/performance`);
...
for (const post of tiktok_posts) {
  ...
  try {
    ...
    if (performance_data) {
      ...
      updateDatabaseWithPerformanceData(performance_data, post, campaign_id, brand_id);
...
async function fetchTikTokData(post, campaign_id, task_id) {
  ...
  try {
    const response = await fetch(`https://open.tiktokapis.com/v2/video/query/?fields=id,share_count,like_count,comment_count,view_count`, options);
    return await response.json();
  } catch (error) {
    console.log("Error w/ TikTok API: " + error);
    // Fetch the initial data from Firebase
    const postsRef = firebase.database().ref(`influencer_campaigns/${campaign_id}/tasks/${task_id}/posts`);
    const postsSnapshot = await postsRef.once('value');
    const postsData = postsSnapshot.val();
    if (postsData) {
      console.log("tiktok api failed: retrieving data from Firebase")
      // Get the last object that starts with "-"
      const postKeys = Object.keys(postsData);
      const lastObjectKey = postKeys.filter(key => key.startsWith('-')).sort().pop();
      if (lastObjectKey) {
        ...
        const performanceRef = firebase.database().ref(`influencer_campaigns/${campaign_id}/tasks/${task_id}/posts/${post.post_id}/performance`);
...
async function fetchBitlyData(post) {
  ...
  const response = await fetch(`https://api-ssl.bitly.com/v4/bitlinks/${shortLink}/clicks/summary?unit=day&units=-1`, {
      headers: {
  ...
  if (response.ok) {
      ...
      console.error('Failed to get data from Bitly API', response.status, response.statusText);
      return 0;
  }
}

function updateDatabaseWithPerformanceData(performance_data, post, campaign_id, brand_id) {
  const performanceRef = firebase.database().ref(`influencer_campaigns/${campaign_id}/tasks/${post.task_id}/posts/${post.post_id}/performance`);
  const newPerformanceRef = performanceRef.push();
  newPerformanceRef.set(performance_data);
  
  const influencerTasksRef = firebase.database().ref(`influencer_tasks/${post.task_id}/posts/${post.post_id}/performance`);
  const newInfluencerTasksRef = influencerTasksRef.push();
  newInfluencerTasksRef.set(performance_data);
  
  const userInfluencerTasksRef = firebase.database().ref(`users/${post.creator_id}/influencer_tasks/${post.task_id}/posts/${post.post_id}/performance`);
  const newUserInfluencerTasksRef = userInfluencerTasksRef.push();
  newUserInfluencerTasksRef.set(performance_data);
  
  const brandInfluencerCampaignsRef = firebase.database().ref(`brands/${brand_id}/influencer_campaigns/${campaign_id}/tasks/${post.task_id}/posts/${post.post_id}/performance`);
  ...
  newBrandInfluencerCampaignsRef.set(performance_data);
      ...
      if (post.media_id) {
        ...
        if (Object.keys(performance_data).length === 0){
         ...
         // Fetch the last added performance object
          const performanceRef = firebase.database().ref(`influencer_campaigns/${campaign_id}/tasks/${post.task_id}/posts/${post.post_id}/performance`);
          const snapshot = await performanceRef.orderByChild('updated').limitToLast(1).once('value');
        ...
        const postSnapshot = await firebase
          .database()
          .ref(`influencer_campaigns/${campaign_id}/tasks/${post.task_id}/posts/${post.post_id}`)
          .once('value');
        ...
        const postRef = firebase
          .database()
          .ref(`influencer_campaigns/${campaign_id}/tasks/${post.task_id}/posts/${post.post_id}/performance`);
    ...
    campaignUpdate = Object.entries(campaignUpdate).reduce((updatedCampaign, [key, value]) => {
      updatedCampaign[key] = key === 'updated' ? value : isNaN(value) ? 0 : value;
    ...
    const brandAnalyticsRef = firebase.database().ref(`brands/${brand_id}/influencer_campaigns/${campaign_id}/historical_analytics`);
    const newBrandAnalyticsRef = brandAnalyticsRef.push();
    newBrandAnalyticsRef.set(campaignUpdate);
    
    const influencerCampaignsAnalyticsRef = firebase.database().ref(`influencer_campaigns/${campaign_id}/historical_analytics`);
    const newInfluencerCampaignsAnalyticsRef = influencerCampaignsAnalyticsRef.push();
  ...
  res.status(200).json({ statuscode: 200, message: "Successfully updated campaign analytics" });
}
   catch (error) {
    console.error("Error finding campaigns for brand", error);
    res.status(500).json({ statuscode: 500, message: "Failed to find campaigns." });
  }
