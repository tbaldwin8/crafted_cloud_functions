    ...
    for (const userId in usersData) {
      if (usersData.hasOwnProperty(userId)) {
        ...
        if (instagramInfo && instagramInfo.instagram_business_account_id && instagramInfo.access_token) {
          const business_account_id = instagramInfo.instagram_business_account_id;
          ...
          try {
            ...
            const genderData = genderApiResponse.data.data[0].total_value.breakdowns[0].results;
            const cityApiResponse = await axios.get(cityApi);
            const cityData = cityApiResponse.data.data[0].total_value.breakdowns[0].results;
            const countryApiResponse = await axios.get(countryApi);
            const countryData = countryApiResponse.data.data[0].total_value.breakdowns[0].results;
            const ageApiResponse = await axios.get(ageApi);
            const ageData = ageApiResponse.data.data[0].total_value.breakdowns[0].results;
            ...
            const usStates = ['alabama', 'alaska', 'arizona', 'arkansas', 'california', 'colorado', 'connecticut', 'delaware', 'florida',
              ...
              'tennessee', 'texas', 'utah', 'vermont', 'virginia', 'washington', 'washington_dc', 'west_virginia', 
              'wisconsin', 'wyoming']
            ...
            for (const result of cityData) {
              ...
              if (usStates.includes(state)) {
                if (stateValues[state]) {
                  stateValues[state].total = (stateValues[state].total || 0) + value;
                  stateValues[state][cityName] = (stateValues[state][cityName] || 0) + value;
                } else {
            ...
            await firebase.database().ref(`users/${userId}/creator_socials/instagram/demographics`).update({
              gender: genderValues,
              city: cityValues,
              states: stateValues,
              age: ageValues,
            ...
            console.error('Failed to fetch and store Instagram demographics data:', error);
